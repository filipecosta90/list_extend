...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv,
                       int argc) {
  if (RedisModule_Init(ctx, "list_extend", 1, REDISMODULE_APIVER_1) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;

  // "write": The command may modify the data set (it may also read from it).
  // "deny-oom": The command may use additional memory and should be denied
  // during out of memory conditions.
  if (RedisModule_CreateCommand(ctx, "list_extend.filter",
                                ListExtendFilter_RedisCommand, "write deny-oom",
